...
const getGraphableAttributes = (jsonData) => {
    ...
    const DataController = {
        ...
        getDataByDateRange: async (req, res) => {
            ...
            return {
              ...
              // Group by sensor
              const groupedBySensor = filteredSelectedData.reduce((acc, item) => {
            item.values.forEach(value => {
              if (!acc[value.sensor]) {
                acc[value.sensor] = [];
              }
              acc[value.sensor].push({ date: item.date, ...value });
            });
            return acc;
              }, {});
              
              res.json(groupedBySensor);
            } catch (error) {
              console.error("Error fetching data:", error);
              res.status(500).json({ error: 'Failed to fetch data by date range' });
            }
    ...
    const extractValue = (entry, filter) => {
        const match = filter.match(/Sensores\[(.+?)\]\.(.+)/);
        if (match) {
            const sensorName = match[1];
            const metric = match[2];
    
            if (entry.Sensores && Array.isArray(entry.Sensores)) {
                return entry.Sensores.map(sensor => {
                    if (sensor.sensor === sensorName && sensor[metric] !== undefined) {
                        return {
                            name: filter,
                            value: sensor[metric]
                        };
